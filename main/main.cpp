...
int g_nCmdShow;
HWND g_mainWnd;//Обект HWND -WIndows Handler _Дескриптор окна
// RGB заливки
int g_FillColor = -1;
int g_RowStart = -1;
int g_ColStart = -1;
...
LRESULT CALLBACK WndProc(HWND, UINT, WPARAM, LPARAM);//Каллбак функция
BOOL InitAppClass();//Функция который регестрирует класс 
BOOL InitWindow();//Инициализация окна
WPARAM StartMessageLoop();//Бесконечный Цикл Сообшение ОС
...
bool loadImage()
{
	...
	// номер ряда возрастает снизу вверх
	for (int row = 0; row < bm.bmHeight; ++row)
	{
		// столбцы, ограницивающие фигуру будем искать по наибольшей ширине незаливочных пикселей в ряду
		int col_min = -1;
		int col_max = -1;
		...
		for (int col = 0; col < bm.bmWidth; ++col)
		{
			...
			// формируем каналы в одно число, чтобы удобней работать
		...
		if (col_min > 0)
		{
			// текущий ряд имеет пиксели фигуры

			// если ряд начала еще не был найден - инициализируем
			if (min_figure_row < 0) min_figure_row = row;

			// также считаем расстояние между столбцами и берем наибольшее из найденных
		...
		{
			// текущий ряд не имеет пикселей фигуры

			// если уже была найдена фигура - ставим послений ряд, -1 потому что текущий ряд уже не содержит пикселей фигуры 
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
	{
		...
		// считаем координаты для вывода картинки g_Width х g_Height в окно r.right х r.bottom
		// описание георметрии на рисунке будет
		// на самом деле мы выводим квадрат, поэтому g_Width = g_Height, но не суть
		...
		// длина и ширина выводимой картинки
		int width = g_Width;
		int height = g_Height;

		if (!(r.right > g_Width && r.bottom > g_Height))
		{
			// если картинка не помещается - нужно пересчитать координаты для вывода

			// тк выводим квадрат - считаем сторону квадрата как наименьшее из измерений среди высоты и ширины окна
			...
			{
				// ограничение по х
				desired_y = r.bottom - l;
			}
			else
			{
				// ограничение по у
				desired_x = r.right / 2 - l / 2;
			}
		}
		...
		// выводим картинку
		// desiredxy - координаты левого верхнего угла картинки
		// width heigth - ширина длина
		StretchBlt(hdc, desired_x, desired_y, width, height, hdcMem, g_ColStart, bitmap.bmHeight - (g_RowStart + g_Height), g_Width, g_Height, SRCCOPY);
